...
def pca(X, Y):
    ...
    for epoch in range(epochs):
        ...
        perf_list.append([acc1, sensibilidade, especificidade, precisao, f1, mg, tempo_treino*1000, tempo_teste*1000])
    # Transforma a lista de resultados em um array Numpy
    PERF = np.array(perf_list)

    return gerar_linha_resultado(
    "PCA (Autoencoder Linear)",
    PERF
)
...
if __name__ == "__main__":
    ...
    df_resultados.to_csv(
        "tabela_resultados_detectores_TC3.csv",
        index=False
    )